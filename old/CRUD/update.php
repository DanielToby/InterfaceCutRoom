...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($brand_err) && empty($TCP1_err) && empty($TCP2_err) && empty($TCL_err) && empty($CLTS_err) && empty($CST_err) && empty($COEF_err) && empty($CS_err) && empty($PMT_err) && empty($CLT_err) && empty($CV_err) && empty($SCST_err) && empty($PST_err) && empty($MST_err) && empty($MLR_err) && empty($DT_err) && empty($SOEF_err) && empty($SSA_err) && empty($DY_err) && empty($SS_err) && empty($ST_err) && empty($CRT_err) && empty($NM_err) && empty($TNR_err) && empty($TCY_err) && empty($STF_err) && empty($CRF_err) && empty($cutter_speed_err)){
        // Prepare an update statement
        $sql = "UPDATE jobs SET name=?, brand=?, cutter_speed=?, TCP1=?, TCP2=?, TCL=?, CLTS=?, CST=?, COEF=?, CS=?, PMT=?, CLT=?, CV=?, TCT=?, CCRC=?, MCMT=?, NCM=?, MCMTS=?, MCT=?, MPMT=?, SCST=?, PST=?, MST=?, MLR=?, DT=?, SOEF=?, SSA=?, DY=?, SS=?, ST=?, CRT=?, NM=?, TNR=?, TCY=?, STF=?, CRF=?, MMT=?, MNT=?, MUT=?, MS=?, MT=?, MRT=?, MLT=?, MCaT=?, MDT=?, XSST=?, XST=?, TST=?, TDT=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssssssssssssssssssssssssssssssssssssssssssssssi", $param_name, $param_brand, $param_cutter_speed, $param_TCP1, $param_TCP2, $param_TCL, $param_CLTS, $param_CST, $param_COEF, $param_CS, $param_PMT, $param_CLT, $param_CV, $param_TCT, $param_CCRC, $param_MCMT, $param_NCM, $param_MCMTS, $param_MCT, $param_MPMT, $param_SCST, $param_PST, $param_MST, $param_MLR, $param_DT, $param_SOEF, $param_SSA, $param_DY, $param_SS, $param_ST, $param_CRT, $param_NM, $param_TNR, $param_TCY, $param_STF, $param_CRF, $param_MMT, $param_MNT, $param_MUT, $param_MS, $param_MT, $param_MRT, $param_MLT, $param_MCaT, $param_MDT, $param_XSST, $param_XST, $param_TST, $param_TDT, $param_id);
